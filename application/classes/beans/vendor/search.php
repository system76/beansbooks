...
@returns vendors ARRAY An array of #Beans_Vendor#.
...
---BEANSENDSPEC---
*/
class Beans_Vendor_Search extends Beans_Vendor {
	...
	public function __construct($data = NULL)
	{
		...
		$this->_vendors = ORM::Factory('entity_vendor')->distinct(TRUE);
	...
	{
		$this->_vendors = $this->_vendors->where('type','=','vendor')->and_where_open();

		$parameter_added = FALSE;

		if( $this->_search_name )
		{
			$this->_vendors = $this->_search_and ? $this->_vendors->and_where_open() : $this->_vendors->or_where_open();
			...
			foreach( explode(' ',$this->_search_name) as $search_name_term )
			{
				if( trim($search_name_term) )
				{
					...
					$this->_vendors = $this->_vendors->or_where('first_name','LIKE','%'.$search_name_term.'%');
					$this->_vendors = $this->_vendors->or_where('last_name','LIKE','%'.$search_name_term.'%');
					$this->_vendors = $this->_vendors->or_where('company_name','LIKE','%'.$search_name_term.'%');
				}
			}

			$this->_vendors = $this->_search_and ? $this->_vendors->and_where_close() : $this->_vendors->or_where_close();
		}
		...
		if( $this->_search_email )
		{
			...
			foreach( explode(' ',$this->_search_email) as $search_email_term )
			{
				...
				{
					$parameter_added = TRUE;
					$this->_vendors = $this->_vendors->or_where('email','LIKE','%'.$search_email_term.'%');
				}
			}
		...
		if( $this->_search_number )
		{
			...
			foreach( explode(' ',$this->_search_number) as $search_number_term )
			{
				...
				{
					$parameter_added = TRUE;
					$this->_vendors = $this->_vendors->or_where('phone_number','LIKE','%'.$search_number_term.'%');
					$this->_vendors = $this->_vendors->or_where('fax_number','LIKE','%'.$search_number_term.'%');
				}
			}
		...
		if( ! $parameter_added )
			$this->_vendors = $this->_vendors->where('id','IS NOT',NULL);

		$this->_vendors = $this->_vendors->and_where_close();
		
		$result_object = $this->_find_vendors();
		...
		return (object)array(
			...
			"vendors" => $this->_return_vendors_array($result_object->vendors),
		);
	}

	protected function _find_vendors()
	{
		...
		$vendors_count = clone($this->_vendors);
		$return_object->total_results = $vendors_count->count_all('entity_vendor.id');
		...
		$return_object->vendors = $this->_vendors->find_all();
