...
class View_Vendors_Purchases extends View_Template {
	
	private $_purchase_payable_accounts;
	function purchase_payable_accounts()
	{
		if( isset($this->_purchase_payable_accounts) )
			return $this->_purchase_payable_accounts;

		$settings = parent::beans_settings();

		$this->_purchase_payable_accounts = parent::all_accounts_chart_flat();

		if( isset($settings->account_default_payable) AND 
			$settings->account_default_payable )
		{
			foreach( $this->_purchase_payable_accounts as $index => $account )
			{
				if( $account['id'] == $settings->account_default_payable )
					$this->_purchase_payable_accounts[$index]['selected'] = TRUE;
				else
					$this->_purchase_payable_accounts[$index]['selected'] = FALSE;
			}
		}
		else
		{
	...
	function purchase_payable_account_default()
	{
		foreach( $this->purchase_payable_accounts() as $account )
			if( $account['selected'] )
				return $account;
